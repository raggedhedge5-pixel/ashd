...
- **Combat log** (own config file, `combatlog.yml` - kept separate from `config.yml` on purpose):
  ...
  *cooldown*, not a ban: pearls get a real vanilla cooldown (the red swirl - genuinely unusable,
  covered from either hand) for `pearl-cooldown-seconds`; mace gets the same cosmetic cooldown swirl
  but - important caveat - vanilla's item-cooldown system doesn't actually gate melee swings, only
  right-click "use" actions, so the mace cooldown has no real effect on attacking with it; it's
  cooldown-in-name for consistency with pearls, nothing more. Damage that a protection plugin (e.g.
